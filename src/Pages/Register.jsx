...
import { registerUser, setAuthUser } from '../utils/auth';
  ...
  return (
    <div className="min-h-screen bg-[#0b132b] text-white flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#111827]/90 p-8 shadow-2xl backdrop-blur-xl">
        <h1 className="text-3xl font-black mb-4">Register for SkyCast</h1>
        <p className="text-sm text-white/50 mb-6">Create your account and start using the weather dashboard.</p>

        {error && <div className="mb-4 rounded-full bg-red-500/10 border border-red-500/20 p-3 text-sm text-red-200">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block text-xs uppercase tracking-[0.15em] text-white/50">Name</label>
          <input
            type="text"
            value={form.name}
          ...
          <button type="submit" className="w-full rounded-full bg-cyan-400 py-3 text-sm font-black text-slate-950 transition hover:bg-cyan-300">Create Account</button>
        </form>

        <p className="mt-6 text-center text-sm text-white/50">
          Already have an account? <Link to="/login" className="text-cyan-300 hover:text-cyan-100">Login here</Link>
        </p>
      </div>
    </div>
  );
